...
var baiVietNoiBat_QS = `select * from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc WHERE trangThai='daxuatban' and ngaydang BETWEEN (SELECT Date_sub(CURRENT_DATE(), INTERVAL WEEKDAY(CURRENT_DATE()) DAY)) AND(SELECT Date_add(CURRENT_DATE(), INTERVAL (6-WEEKDAY(CURRENT_DATE())) DAY)) ORDER BY luotxem Asc limit 10`;
