...
var full_text_search_QS = (key, limit, offset) => `SELECT * FROM baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc WHERE cm.daXoa=0 and bv.trangThai='daxuatban'  and bv.daXoa=false and round(MATCH(tieuDe,moTa, noiDung) AGAINST (N'${key}' IN BOOLEAN MODE),5)>0  order by premium DESC , ngaydang DESC limit ${limit} offset ${offset}`;
var count_full_text_search_QS = key => `SELECT count(*) FROM baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc WHERE cm.daXoa=0 and bv.trangThai='daxuatban'  and bv.daXoa=false and round(MATCH(tieuDe,moTa, noiDung) AGAINST (N'${key}' IN BOOLEAN MODE),5)>0 `;
