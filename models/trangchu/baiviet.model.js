...
var baiVietNoiBat_QS = `select bv.tieuDe, bv.idBaiViet, bv.anhDaiDien, bv.moTa, bv.ngayDang, cm.*, nd.butDanh from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc join nguoidung nd on nd.username=bv.nguoiDang WHERE bv.trangThai='daxuatban' and bv.daXoa=false and ngaydang BETWEEN (SELECT Date_sub(CURRENT_DATE(), INTERVAL WEEKDAY(CURRENT_DATE()) DAY)) AND(SELECT Date_add(CURRENT_DATE(), INTERVAL (7-WEEKDAY(CURRENT_DATE())) DAY)) ORDER BY luotxem Asc limit 10`;
var baiVietXemNhieuNhat_QS = (limit, offset) => `select bv.tieuDe, bv.idBaiViet, bv.anhDaiDien, bv.moTa, bv.ngayDang, cm.*, nd.butDanh from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc join nguoidung nd on nd.username=bv.nguoiDang where bv.trangThai='daxuatban' and bv.daXoa=false order by luotxem DESC ` + limitString(limit, offset);
var top10ChuyenMuc_QS = `select bv.tieuDe, bv.idBaiViet, bv.anhDaiDien, bv.moTa, bv.ngayDang, cm.*, nd.butDanh from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc join nguoidung nd on nd.username=bv.nguoiDang where bv.trangThai='daxuatban'  and bv.daXoa=false order by ngaydang asc limit 10`; //
var baiVietMoiNhat_QS = (limit, offset) => `select bv.tieuDe, bv.idBaiViet, bv.anhDaiDien, bv.moTa, bv.ngayDang, cm.*, nd.butDanh from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc join nguoidung nd on nd.username=bv.nguoiDang where bv.trangThai='daxuatban'  and bv.daXoa=false ` + orderByDate + limitString(limit, offset);
var baiVietCungChuyenMuc_QS = (id, limit, offset) => `select * from baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc  where bv.trangThai='daxuatban' and bv.daXoa=false and bv.idChuyenMuc=${id} order by ngaydang asc ` + limitString(limit, offset);
var limitString = (limit, offset) => `limit ${limit} offset ${offset}`;
var full_text_search_QS = (key, limit, offset) => `SELECT * FROM baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc WHERE bv.trangThai='daxuatban'  and bv.daXoa=false and round(MATCH(tieuDe,moTa, noiDung) AGAINST (N'${key}' IN BOOLEAN MODE),5)>0  order by premium DESC , ngaydang DESC limit ${limit} offset ${offset}`;
var count_full_text_search_QS = key => `SELECT count(*) FROM baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc WHERE bv.trangThai='daxuatban'  and bv.daXoa=false and round(MATCH(tieuDe,moTa, noiDung) AGAINST (N'${key}' IN BOOLEAN MODE),5)>0 `;
var tongBaiVietChuaTag_QS = (idTag) => `select count(*) as tong from thuoctag  join baiviet on thuoctag.idBaiViet=baiviet.idBaiViet join chuyenmuc on baiviet.idChuyenMuc=chuyenmuc.idChuyenMuc where baiviet.trangThai='daxuatban'  and baiviet.daXoa=false and thuoctag.idTag=${idTag}`;
var baiVietChuaTag_QS = (idTag, limit, offset) => `select * from thuoctag  join baiviet on thuoctag.idBaiViet=baiviet.idBaiViet join chuyenmuc on baiviet.idChuyenMuc=chuyenmuc.idChuyenMuc  where baiviet.trangThai='daxuatban'  and baiviet.daXoa=false and thuoctag.idTag=${idTag} order by premium DESC , ngaydang DESC ` + limitString(limit, offset);
var single = id => db.load(`SELECT bv.*, cm.tenChuyenMuc, nguoidung.butDanh FROM baiviet as bv JOIN chuyenmuc as cm ON bv.idChuyenMuc=cm.idChuyenMuc  join nguoidung on nguoidung.username=bv.nguoiDang WHERE bv.trangThai='daxuatban'  and bv.daXoa=false and idBaiViet= ${id}`);
...
var baiVietXemNhieuNhatChuyenMuc = (id) => {
    ...
    WHERE trangThai='daxuatban'  and bv.daXoa=false and bv.idChuyenMuc = ${id} and luotxem=(
...
var tongBaiVietChuyenMuc = id => db.load(`Select count(*) as tong from (SELECT cm2.idChuyenMuc FROM chuyenmuc cm join chuyenmuc cm2 on cm.idChuyenMuc=cm2.chuyenMucCha
    Where cm.idChuyenMuc=${id} UNION select ${id}) as id join baiviet as bv on id.idChuyenMuc=bv.idChuyenMuc where bv.trangThai='daxuatban'  and bv.daXoa=false`);
