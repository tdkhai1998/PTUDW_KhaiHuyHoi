...
module.exports = {
    allbyuserStt: (user, stt, limit, offset) => {
        return db.load(`select * from baiviet, chuyenmuc where baiviet.idChuyenMuc = chuyenmuc.idChuyenMuc and  baiviet.nguoiDang ='${user}' and trangThai='${stt}' order by ngayDang DESC limit ${limit} offset ${offset}`);
    },
    countAllByUserStt: (user, stt) => db.load(`select count(*) count from baiviet, chuyenmuc where baiviet.idChuyenMuc = chuyenmuc.idChuyenMuc and  baiviet.nguoiDang ='${user}' and trangThai='${stt}' order by ngayDang DESC`),
    all: (idChuyenMuc) => {
